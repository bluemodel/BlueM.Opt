...
namespace BlueM.Opt.Algos.MetaEvo
{
    ...
    class Algos
    {
        ...
        public Algos(ref BlueM.Opt.Common.Settings settings_input, int individuum_id_input, ref BlueM.Opt.Diagramm.Monitor monitor_input)
        {
        ...
        public void set_algos(string algos2use_input) 
        {
            ...
            BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Using Algos: " + algos2use_input);
        }

        //Neue Generation erzeugen
        public void newGeneration(ref BlueM.Opt.Common.Individuum_MetaEvo[] genpool_input, ref BlueM.Opt.Common.Individuum_MetaEvo[] new_generation_input, ref BlueM.Opt.Common.Individuum_MetaEvo[] wastepool_input)
        {
            int startindex;
        ...
        private void build_individuals(ref BlueM.Opt.Common.Individuum_MetaEvo[] genpool_input, ref BlueM.Opt.Common.Individuum_MetaEvo[] new_generation_input, ref BlueM.Opt.Common.Individuum_MetaEvo[] wastepool_input, int algo_id, int startindex_input)
        {
            ...
            switch (algofeedbackarray[algo_id].name)
            {
                ...
                case "Zufällige Einfache Mutation":
                    {
                        BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Buliding " + numberindividuums + " Individuums with " + algo_id + ":'" + algofeedbackarray[algo_id].name + "'...done");
                        ...
                        for (int i = 0; i < numberindividuums; i++)
                        {
                            ...
                            //Zurückspeichern
                            new_generation_input[startindex + i] = new BlueM.Opt.Common.Individuum_MetaEvo("MetaEvo", individuum_id, numberoptparas);
                ...
                case "Ungleichverteilte Mutation":
                    {
                        BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Building " + numberindividuums + " Individuums with " + algo_id + ":'" + algofeedbackarray[algo_id].name + "'...done");
                ...
                case "Totaler Zufall":
                    {
                        ...
                        for (int k = 0; k < numberindividuums; k++)
                        {
                            ...
                            //Zurückspeichern
                            new_generation_input[startindex + k] = new BlueM.Opt.Common.Individuum_MetaEvo("MetaEvo", individuum_id, numberoptparas);
                ...
                case "Hook and Jeeves":  //VGL: Syrjakow S.95f
                    {
                        ...
                        if (firstrun)
                        {
                            ...
                            BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: Starting precalculation of weights");
                            ...
                            {
                                Penalties_tmp = genpool_input[i].PrimObjectives;
                        ...
                        BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: Starting Calculation of " + numberindividuums + " Processes");
                        for (int i = 0; i < numberindividuums; i++)
                        {
                            ...
                            while(run)
                            {
                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": HJ-state: " + nextstate);
                                switch (nextstate)
                                {
                                    case (-1): //-1: Schrittweiten definieren
                                        {
                                            ...
                                            genpool_input[i].feedbackdata = new double[3, Math.Max(genpool_input[0].PrimObjectives.Length, Math.Max(numberoptparas, 3))];
                                            for (int j = 0; j < numberoptparas; j++)
                                            {
                                                //Wichtung berechnen
                                                ...
                                                if (j < numberpenalties)
                                                {
                                                    ...
                                                    {
                                                        genpool_input[i].feedbackdata[0, j] += (weightsinfo[1, k] - genpool_input[i].PrimObjectives[k]) / (weightsinfo[1, k] - weightsinfo[0, k]);
                                                    }
                                                    ...
                                                    BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Weight for Penalty[" + j + "]: " + genpool_input[i].feedbackdata[0, j]);
                                                }
                                    ...
                                    case (0): //0: Tast0
                                        {
                                            ...
                                            new_generation_input[i * 3 + 1] = new BlueM.Opt.Common.Individuum_MetaEvo("MetaEvo", individuum_id, numberoptparas);
                                    ...
                                    case (1): //1: Tast1: Vergleich -Opt 
                                        {
                                            ...
                                            {
                                                double[] new_penalties = new_generation_input[i * 3].PrimObjectives;
                                                double[] new2_penalties = new_generation_input[i * 3 + 1].PrimObjectives;
                                                ...
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": WightedPenalties: BestIndividuum " + new_generation_input[i * 3].ID + " = " + weighted_penaltie_new + " / NewIndividuum " + new_generation_input[i * 3 + 1].ID + " = " + weighted_penaltie_new2);
                                    ...
                                    case (4): //4: Tast4
                                        { 
                                            ...
                                            new_generation_input[i * 3].set_toSimulate(false);
                                            BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum: " + new_generation_input[i * 3].ID);
                                            nextstate = 5;
                                            break;
                                        }
                                    case (5): //5: Tast5
                                        {
                                            ...
                                            {
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Optparameter " + (genpool_input[i].feedbackdata[2, 1]) + " von " + numberoptparas + " mit der vorgegebenen Schrittweite getestet");  
                                                genpool_input[i].feedbackdata[2, 1]++;
                                                nextstate = 0;
                                            }
                                            else
                                            {
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Optparameter " + numberoptparas + " von " + numberoptparas + " mit der vorgegebenen Schrittweite getestet");  
                                                genpool_input[i].feedbackdata[2, 1] = 0;//zu variierender optparameter [1]
                                                nextstate = 10;
                                            }
                                            break;
                                    ...
                                    case (10): //10: Vergleich der Funktionswerte
                                        {
                                            double[] gen_penalties = genpool_input[i].PrimObjectives;
                                            double[] new_penalties = new_generation_input[i * 3].PrimObjectives;
                                            ...
                                            {
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum " + new_generation_input[i * 3].ID + " Penaltie: " + weighted_penaltie_new + " ist besser als Basisindividuum " + genpool_input[i].ID + " Penaltie: " + weighted_penaltie_gen);
                                                nextstate = 20;
                                            }
                                            else
                                            {
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum " + new_generation_input[i * 3].ID + " Penaltie: " + weighted_penaltie_new + " ist NICHT besser als Basisindividuum " + genpool_input[i].ID + " Penaltie: " + weighted_penaltie_gen);
                                                nextstate = 30;
                                            }
                                            break;
                                        }
                                    case (20): //20: Extrapolation
                                        {
                                            ...
                                            BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum "+individuum_id+" mit Extrapolationsschritt auf " + tmp + ")");
                                            ...
                                            new_generation_input[i * 3] = new BlueM.Opt.Common.Individuum_MetaEvo("MetaEvo", individuum_id, numberoptparas);
                                    ...
                                    case (30): //30: Extrapolation zurück
                                        {
                                            ...
                                            {
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Extrapolation zurückgehen");
                                                new_generation_input[i * 3] = genpool_input[i].Clone_MetaEvo();
                                                new_generation_input[i * 3].ID = individuum_id;
                                                individuum_id++;
                                    ...
                                    case (40): //40: Schrittweite halbieren oder Goal
                                        {
                                            ...
                                            for (int j = 0; j < numberoptparas; j++)
                                            {
                                                ...
                                                BlueM.Opt.Common.Log.AddMessage(BlueM.Opt.Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Schrittweite[" + j + "] von " + genpool_input[i].feedbackdata[1, j] * 2 + " auf " + genpool_input[i].feedbackdata[1, j]);
                                            }
