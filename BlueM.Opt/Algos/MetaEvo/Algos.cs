...
namespace BlueM.Opt.Algos.MetaEvo
{
    ...
    class Algos
    {
        ...
        public void set_algos(string algos2use_input) 
        {
            ...
            Common.Log.AddMessage(Common.Log.levels.info, "Algos: Using Algos: " + algos2use_input);
        }
        ...
        private void build_individuals(ref BlueM.Opt.Common.Individuum_MetaEvo[] genpool_input, ref BlueM.Opt.Common.Individuum_MetaEvo[] new_generation_input, ref BlueM.Opt.Common.Individuum_MetaEvo[] wastepool_input, int algo_id, int startindex_input)
        {
            ...
            switch (algofeedbackarray[algo_id].name)
            {
                ...
                case "Zufällige Einfache Mutation":
                    {
                        Common.Log.AddMessage(Common.Log.levels.info, "Algos: Buliding " + numberindividuums + " Individuums with " + algo_id + ":'" + algofeedbackarray[algo_id].name + "'...done");
                ...
                case "Ungleichverteilte Mutation":
                    {
                        Common.Log.AddMessage(Common.Log.levels.info, "Algos: Building " + numberindividuums + " Individuums with " + algo_id + ":'" + algofeedbackarray[algo_id].name + "'...done");
                ...
                case "Hook and Jeeves":  //VGL: Syrjakow S.95f
                    {
                        ...
                        if (firstrun)
                        {
                            ...
                            Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: Starting precalculation of weights");
                        ...
                        Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: Starting Calculation of " + numberindividuums + " Processes");
                        for (int i = 0; i < numberindividuums; i++)
                        {
                            ...
                            while(run)
                            {
                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": HJ-state: " + nextstate);
                                switch (nextstate)
                                {
                                    case (-1): //-1: Schrittweiten definieren
                                        {
                                            ...
                                            for (int j = 0; j < numberoptparas; j++)
                                            {
                                                ...
                                                if (j < numberpenalties)
                                                {
                                                    ...
                                                    Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Weight for Penalty[" + j + "]: " + genpool_input[i].feedbackdata[0, j]);
                                                }
                                    ...
                                    case (1): //1: Tast1: Vergleich -Opt 
                                        {
                                            if (new_generation_input[i * 3 + 1].get_status() == "true")
                                            {
                                                ...
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": WightedPenalties: BestIndividuum " + new_generation_input[i * 3].ID + " = " + weighted_penaltie_new + " / NewIndividuum " + new_generation_input[i * 3 + 1].ID + " = " + weighted_penaltie_new2);
                                    ...
                                    case (4): //4: Tast4
                                        { 
                                            ...
                                            new_generation_input[i * 3].set_toSimulate(false);
                                            Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum: " + new_generation_input[i * 3].ID);
                                            nextstate = 5;
                                            break;
                                        }
                                    case (5): //5: Tast5
                                        {
                                            ...
                                            {
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Optparameter " + (genpool_input[i].feedbackdata[2, 1]) + " von " + numberoptparas + " mit der vorgegebenen Schrittweite getestet");  
                                                genpool_input[i].feedbackdata[2, 1]++;
                                                nextstate = 0;
                                            }
                                            else
                                            {
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Optparameter " + numberoptparas + " von " + numberoptparas + " mit der vorgegebenen Schrittweite getestet");  
                                                genpool_input[i].feedbackdata[2, 1] = 0;//zu variierender optparameter [1]
                                                nextstate = 10;
                                            }
                                            break;
                                    ...
                                    case (10): //10: Vergleich der Funktionswerte
                                        {
                                            ...
                                            {
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum " + new_generation_input[i * 3].ID + " Penaltie: " + weighted_penaltie_new + " ist besser als Basisindividuum " + genpool_input[i].ID + " Penaltie: " + weighted_penaltie_gen);
                                                nextstate = 20;
                                            }
                                            else
                                            {
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum " + new_generation_input[i * 3].ID + " Penaltie: " + weighted_penaltie_new + " ist NICHT besser als Basisindividuum " + genpool_input[i].ID + " Penaltie: " + weighted_penaltie_gen);
                                                nextstate = 30;
                                            }
                                            break;
                                        }
                                    case (20): //20: Extrapolation
                                        {
                                            ...
                                            Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Neues BestIndividuum "+individuum_id+" mit Extrapolationsschritt auf " + tmp + ")");
                                    ...
                                    case (30): //30: Extrapolation zurück
                                        {
                                            ...
                                            {
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Extrapolation zurückgehen");
                                                new_generation_input[i * 3] = genpool_input[i].Clone_MetaEvo();
                                                new_generation_input[i * 3].ID = individuum_id;
                                                individuum_id++;
                                    ...
                                    case (40): //40: Schrittweite halbieren oder Goal
                                        {
                                            ...
                                            for (int j = 0; j < numberoptparas; j++)
                                            {
                                                ...
                                                Common.Log.AddMessage(Common.Log.levels.info, "Algos: Hook and Jeeves: BaseIndividuum " + genpool_input[i].ID + ": Schrittweite[" + j + "] von " + genpool_input[i].feedbackdata[1, j] * 2 + " auf " + genpool_input[i].feedbackdata[1, j]);
                                            }
